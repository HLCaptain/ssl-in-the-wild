...
class BirdsDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if not self.data_train and not self.data_val and not self.data_test:
            ...
            trainset = ImageFolderCustom(class_csv=birds_100_csv_path, targ_dir=train_dir,transform=self.transforms)
            testset = ImageFolderCustom(class_csv=birds_100_csv_path, targ_dir=test_dir, transform=self.transforms)
            valset = ImageFolderCustom(class_csv=birds_100_csv_path, targ_dir=val_dir, transform=self.transforms)
